...
export default function GuessRow({ building, target }) {
    ...
    let fields = [
        { value: building.name + " (" + building.acronym + ")", isCorrect: (building.name === target.name? 1 : 0) },
        { value: building.faculty.join(", "), isCorrect: 1 },
        { value: building.numFloors + (building.numFloors > target.numFloors ? " ↓" : building.numFloors < target.numFloors ? " ↑" : ""), isCorrect: (building.numFloors === target.numFloors? 1 : 0) },
        { value: building.builtYear + (building.builtYear > target.builtYear ? " ↓" : building.builtYear < target.builtYear ? " ↑" : ""), isCorrect: (building.builtYear === target.builtYear? 1 : 0) },
        { value: building.servesFood ? "Yes" : "No", isCorrect: (building.servesFood === target.servesFood? 1 : 0) },
    ];

    // set the faculty colours
    const buildingFacSet = new Set(building.faculty);
    const targetFacSet = new Set(target.faculty);
    if (buildingFacSet.size === targetFacSet.size && [...buildingFacSet].every(f => targetFacSet.has(f))) {
